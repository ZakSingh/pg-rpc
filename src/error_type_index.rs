...
pub fn collect_error_type_oids(db: &mut Client, errors_config: &ErrorsConfig) -> anyhow::Result<Vec<OID>> {
...
pub fn generate_error_payload_structs(
    ...
    for error_type in error_index.values() {
        ...
        let fields: Vec<TokenStream> = error_type.fields.iter().map(|field| {
            let field_name = sql_to_rs_ident(&field.name, CaseType::Snake);
            let field_type = ty_index.deref()
                .get(&field.type_oid)
...
pub fn generate_error_type_enum(error_index: &ErrorTypeIndex) -> TokenStream {
    ...
    let variants: Vec<TokenStream> = error_index.values().map(|error_type| {
    ...
    let from_json_arms: Vec<TokenStream> = error_index.values().map(|error_type| {
