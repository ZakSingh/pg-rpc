...
fn test_sql_language_function_parsing() {
    with_isolated_database_and_container(|client, _container, _conn_string| {
        ...
        assert!(generated.contains("get_user_by_email"), "Should contain SQL SELECT function");
        assert!(generated.contains("update_user_name"), "Should contain SQL UPDATE function");
        assert!(generated.contains("insert_user"), "Should contain PL/pgSQL function");
        ...
        assert!(error_enum_count >= 1, "Should have at least one error enum for functions touching constrained tables");
...
fn test_sql_function_with_triggers() {
    with_isolated_database_and_container(|client, _container, _conn_string| {
        ...
        println!("Generated code for SQL function with triggers:\n{}", generated);
        
        // Check that the SQL delete function was generated
        assert!(generated.contains("delete_user"), "Should contain SQL delete function");
        
        // The delete function should have error handling due to the trigger that raises exceptions
        assert!(generated.contains("DeleteUserError") || generated.contains("delete_user_error"), 
...
fn test_sql_function_constraint_exceptions() {
    with_isolated_database_and_container(|client, _container, conn_string| {
        ...
        println!("Generated code for SQL functions with constraints:\n{}", generated);
        
        // Verify that error enums are generated for SQL functions
        assert!(generated.contains("CreateUserError"), "Should have error enum for create_user");
        assert!(generated.contains("UpdateUserAgeError"), "Should have error enum for update_user_age");
        assert!(generated.contains("DeleteUserError"), "Should have error enum for delete_user");
        
        // Verify that constraint variants are included
        assert!(generated.contains("UsersConstraint"), "Should have users constraint enum");
        ...
        assert!(errors_content.contains("UsersEmailKey"), "Should have unique constraint variant");
        assert!(errors_content.contains("UsersNameCheck"), "Should have name check constraint");
        assert!(errors_content.contains("UsersAgeCheck"), "Should have age check constraint");
