...
fn test_constraint_discovery() {
    with_isolated_database(|client| {
        ...
        let constraints: Vec<(String, String, String)> = rows
            .iter()
            .map(|row| {
                (
        ...
        let account_constraints: Vec<_> = constraints
            .iter()
            .filter(|(_, _, table)| table == "account")
            .collect();

        // Should have primary key, unique, and check constraints
        assert!(account_constraints
            .iter()
            .any(|(_, ctype, _)| ctype == "PRIMARY KEY"));
        assert!(account_constraints
            .iter()
            .any(|(_, ctype, _)| ctype == "UNIQUE"));
        assert!(account_constraints
            .iter()
            .any(|(_, ctype, _)| ctype == "CHECK"));

        let post_constraints: Vec<_> = constraints
            .iter()
        ...
        assert!(post_constraints
            .iter()
            .any(|(_, ctype, _)| ctype == "FOREIGN KEY"));
        assert!(post_constraints
            .iter()
            .any(|(_, ctype, _)| ctype == "CHECK"));
    });
...
fn test_function_discovery() {
    with_isolated_database(|client| {
        ...
        let functions: Vec<(String, String, String)> = rows
            .iter()
            .map(|row| {
                (
        ...
        assert!(functions
            .iter()
            .any(|(name, schema, _)| name == "get_account_by_email" && schema == "api"));
    });
}
...
fn test_enum_type_discovery() {
    with_isolated_database(|client| {
        ...
        let enums: Vec<(String, i8, String)> = rows
            .iter()
            .map(|row| {
                (
                    row.get("typname"),
        ...
        assert!(enums
            .iter()
            .any(|(name, _, schema)| name == "role" && schema == "public"));
    });
}
...
fn test_composite_type_discovery() {
    with_isolated_database(|client| {
        ...
        let composites: Vec<(String, i8, String)> = rows
            .iter()
            .map(|row| {
                (
                    row.get("typname"),
        ...
        assert!(composites
            .iter()
            .any(|(name, _, schema)| name == "account" && schema == "public"));
        assert!(composites
            .iter()
            .any(|(name, _, schema)| name == "post" && schema == "public"));
    });
}
