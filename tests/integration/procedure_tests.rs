...
fn test_procedure_with_no_parameters() {
    with_isolated_database_and_container(|client, _container, conn_string| {
        ...
        assert!(public_content.contains("CALL public.simple_procedure"),
            "Should generate CALL statement for procedure");
        assert!(public_content.contains("Result<(), SimpleProc"),
...
fn test_procedure_with_in_parameters() {
    with_isolated_database_and_container(|client, _container, conn_string| {
        ...
        assert!(has_function, "Should generate the update_user_status function");
        ...
        if !has_call {
            ...
            if let Some(start) = public_content.find("pub async fn update_user_status") {
                let snippet = &public_content[start..start.saturating_add(500).min(public_content.len())];
                println!("{}", snippet);
            }
        }
...
fn test_procedure_with_single_inout_parameter() {
    with_isolated_database_and_container(|client, _container, conn_string| {
        ...
        // Verify the return type is the INOUT parameter type
        assert!(public_content.contains("Result<i32, GetNextIdError>"),
            "Should return the INOUT parameter type directly");
        assert!(public_content.contains("row.try_get(0)"),
...
fn test_procedure_with_multiple_inout_parameters() {
    with_isolated_database_and_container(|client, _container, conn_string| {
        ...
        assert!(public_content.contains("struct GetUserInfoRow"),
            "Should generate struct for multiple INOUT parameters");
        assert!(public_content.contains("p_username: String"),
            "Should have username field in struct");
        assert!(public_content.contains("p_email: String"),
            "Should have email field in struct");
        assert!(public_content.contains("p_active: bool"),
            "Should have active field in struct");
        assert!(public_content.contains("Result<GetUserInfoRow, GetUserInfoError>"),
...
fn test_procedure_with_inout_parameters() {
    with_isolated_database_and_container(|client, _container, conn_string| {
        ...
        assert!(public_content.contains("p_value: i32"),
            "Should have value as input parameter");
        assert!(public_content.contains("Result<i32, DoubleValueError>"),
...
fn test_procedure_error_handling() {
    with_isolated_database_and_container(|client, _container, conn_string| {
        ...
        assert!(public_content.contains("enum ValidateInputError"),
            "Should generate error enum for procedure");
        assert!(public_content.contains("P0001"),
            "Should detect default error code");
        assert!(public_content.contains("23514") || public_content.contains("CheckViolation"),
